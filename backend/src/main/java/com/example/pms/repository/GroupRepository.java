...
public class GroupRepository {
    ...
    private boolean existsByNameInClassAndSemester(String groupName, int classId, int semesterId) {
        return existsByNameInClassAndSemester(groupName, classId, semesterId, null);
    }

    private boolean existsByNameInClassAndSemester(String groupName, int classId, int semesterId, Integer excludeGroupId) {
        try {
            String sql = "SELECT COUNT(*) FROM Groups " +
                    "WHERE ClassID = ? AND SemesterID = ? " +
                    "AND LOWER(LTRIM(RTRIM(GroupName))) = LOWER(LTRIM(RTRIM(?))) " +
                    "AND (? IS NULL OR GroupID <> ?)";
            Integer count = db.queryForObject(sql, Integer.class, classId, semesterId, groupName, excludeGroupId, excludeGroupId);
    ...
    public int updateGroupName(int groupId, String groupName) {
        try {
            ...
            if (existsByNameInClassAndSemester(normalizedGroupName, group.getClassId(), group.getSemesterId(), groupId)) {
                return -2;
            }

            String sql = "UPDATE Groups SET GroupName = ? WHERE GroupID = ?";
            return db.update(sql, normalizedGroupName, groupId);
        } catch (Exception ex) {
            ex.printStackTrace();
            return 0;
        }
