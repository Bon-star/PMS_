...
public class ProjectRepository {
        ...
        synchronized (this) {
            ...
            db.execute(
                    "IF COL_LENGTH('ProjectTemplates', 'ImageUrl') IS NULL " +
                            ...
                            "END");
            db.execute(
                    "IF COL_LENGTH('Projects', 'RequirementFilePath') IS NULL " +
                            "BEGIN " +
                            "   ALTER TABLE Projects ADD RequirementFilePath NVARCHAR(500) NULL; " +
                            "END");
            db.execute(
                    "IF COL_LENGTH('Projects', 'RequirementFileName') IS NULL " +
                            "BEGIN " +
                            "   ALTER TABLE Projects ADD RequirementFileName NVARCHAR(255) NULL; " +
                            "END");
            schemaEnsured = true;
        }
    }
    ...
    private Project mapProject(java.sql.ResultSet rs) throws java.sql.SQLException {
        ...
        p.setRequirementFilePath(rs.getString("RequirementFilePath"));
        p.setRequirementFileName(rs.getString("RequirementFileName"));
    ...
    private String overviewSelect() {
        return "SELECT " +
                ...
                "proj.TemplateImageUrl, proj.RequirementFilePath, proj.RequirementFileName, proj.SourceCodeUrl, proj.DocumentUrl, proj.SubmissionDate, proj.StartDate, proj.EndDate, " +
                ...
                "          p.ApprovalStatus, p.RejectReason, p.RequirementFilePath, p.RequirementFileName, p.SourceCodeUrl, p.DocumentUrl, p.SubmissionDate, " +
    ...
    private String projectSelect() {
        return "SELECT " +
                ...
                "pt.ImageUrl AS TemplateImageUrl, p.RequirementFilePath, p.RequirementFileName, p.SourceCodeUrl, p.DocumentUrl, p.SubmissionDate, pa.StartDate, pa.EndDate, " +
    ...
    public List<Project> findHistoryByStudent(int studentId) {
        ensureSchema();
        String sql = projectSelect() +
                "INNER JOIN Group_Members gm ON gm.GroupID = g.GroupID " +
                "WHERE gm.StudentID = ? AND gm.IsActive = 1 " +
                "ORDER BY ISNULL(sem.StartDate, CAST(pa.StartDate AS DATE)) DESC, ISNULL(pa.EndDate, pa.StartDate) DESC, p.ProjectID DESC";
        return db.query(sql, (rs, rn) -> mapProject(rs), studentId);
    }

    public Project findHistoryProjectForStudent(int projectId, int studentId) {
        ensureSchema();
        try {
            String sql = projectSelect().replaceFirst("^SELECT ", "SELECT TOP 1 ") +
                    "INNER JOIN Group_Members gm ON gm.GroupID = g.GroupID " +
                    "WHERE p.ProjectID = ? AND gm.StudentID = ? AND gm.IsActive = 1";
            return db.queryForObject(sql, (rs, rn) -> mapProject(rs), projectId, studentId);
        } catch (EmptyResultDataAccessException ex) {
            return null;
        }
    }
